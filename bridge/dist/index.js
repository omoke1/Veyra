...
const subgraph_1 = require("./polymarket/subgraph");
dotenv.config();
...
class BridgeService {
    constructor(config) {
        ...
        this.signer = new ethers_1.ethers.Wallet(config.bridgePrivateKey, this.provider);
        // Initialize Polygon provider if provided (for Polymarket)
        if (config.polygonRpcUrl) {
            this.polygonProvider = new ethers_1.ethers.JsonRpcProvider(config.polygonRpcUrl);
        }
        // Initialize contracts
        this.umaOracle = new ethers_1.ethers.Contract(config.umaOracleAddress, UMA_ORACLE_ABI, this.provider);
        this.gnosisConditionalTokens = new ethers_1.ethers.Contract(config.gnosisConditionalTokensAddress, GNOSIS_CONDITIONAL_TOKENS_ABI, this.provider);
        this.umaAdapter = new ethers_1.ethers.Contract(config.umaAdapterAddress, UMA_ADAPTER_ABI, this.signer);
        this.gnosisAdapter = new ethers_1.ethers.Contract(config.gnosisAdapterAddress, GNOSIS_ADAPTER_ABI, this.signer);
        this.vpoAdapter = new ethers_1.ethers.Contract(config.vpoAdapterAddress, VPO_ADAPTER_ABI, this.provider);
        // Initialize Polymarket clients
        if (config.polymarketApiKey || process.env.POLYMARKET_API_KEY) {
            this.polymarketClient = new client_1.PolymarketClient({
        ...
        this.polymarketSubgraph = new subgraph_1.PolymarketSubgraph();
    }
    /**
    ...
    async startUMAMonitor() {
        ...
        this.umaOracle.on("AssertionMade", async (assertionId, claim, asserter, callbackRecipient, escalationManager, liveness, currency, bond, identifier, event) => {
            ...
            try {
                ...
                const data = ethers_1.ethers.AbiCoder.defaultAbiCoder().encode(["bytes32", "address", "uint64"], [identifier, currency, liveness]);
                const tx = await this.umaAdapter.handleAssertion(assertionId, claim, data);
                const receipt = await tx.wait();
                console.log(`   ✅ Handled via UMAAdapter: ${receipt.hash}`);
                // Find the requestId from the event
                const handledEvent = receipt.logs.find((log) => {
                    try {
    ...
    async startGnosisMonitor() {
        ...
        this.gnosisConditionalTokens.on("ConditionPreparation", async (conditionId, oracle, questionId, outcomeSlotCount, event) => {
            ...
            try {
                ...
                const data = ethers_1.ethers.AbiCoder.defaultAbiCoder().encode(["bytes32", "address"], [questionId, oracle]);
                const tx = await this.gnosisAdapter.handleCondition(conditionId, questionId, outcomeSlotCount, data);
                const receipt = await tx.wait();
     ...
     */
    async monitorVPOFulfillment(requestId, externalId, type) {
        ...
        this.vpoAdapter.once("VerificationFulfilled", async (fulfilledRequestId, attestationCid, outcome, metadata) => {
     ...
     */
    async submitUMAOutcome(requestId, assertionId) {
        ...
        try {
            ...
            const tx = await this.umaAdapter.submitOutcomeToUMA(requestId, claim, liveness, currency, bond, identifier);
            const receipt = await tx.wait();
     ...
     */
    async submitGnosisOutcome(requestId, conditionId) {
        ...
        try {
            ...
            const tx = await this.gnosisAdapter.resolveCondition(requestId, outcomeSlotCount);
            const receipt = await tx.wait();
            ...
            try {
                ...
                if (markets.length > 0) {
                    ...
                    for (const market of markets) {
                        ...
                        try {
                            const data = ethers_1.ethers.AbiCoder.defaultAbiCoder().encode(["bytes32", "address"], [market.id, market.condition.oracle]);
                            const tx = await this.gnosisAdapter.handleCondition(conditionId, market.id, // Use market ID as question ID
                            BigInt(market.condition.outcomeSlotCount), data);
                            const receipt = await tx.wait();
                            console.log(`   ✅ Handled via GnosisAdapter: ${receipt.hash}`);
                            // Find the requestId from the event
                            const handledEvent = receipt.logs.find((log) => {
